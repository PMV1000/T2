...
function Cart() {
  ...
  const [selectProduct,setSP]=useState([])
  ...
  const addTocart= async (Id,quan,type='')=>{
    ...
    console.log(quan,"inF")
      ...
      const exist =  cart.products.find(item=>item.productId+'' === Id+'')
      console.log(exist)
    if(!exist){
    const Newdata= {
      ...cart,
      products:[...cart.products,{productId:Id,quantity:quan}]
    }
    setCart(Newdata)
    setId('')
  await fetch (`http://localhost:5000/carts/1`,{
    ...
    products: cart.products.map(item => 
      item.productId === Id
        ? {...item, quantity:( (type === "change")?(quan):(parseInt(item.quantity,10) + quan)) }
        : { ...item }
    )
  } 
  // sử dụng addToCart trường hợp sản phẩm đã có trong giỏ hàng như hàm change Quan vì có một số điểm tương đồng
 ...
 Navigate('/cart')
  ...
  const RemoveFromCart = async (id)=>{
    ...
    const NDT = {...cart,products:cart.products.filter(item=>item.productId+'' !== id+'')}
   ...
   const RM = document.querySelector('#RMALL')
    if(type){
      if(type==="tick"){
        ...
        setSP(selectProduct=>[...selectProduct,{id:id,quantity:tickElement.getAttribute('data-quantity')}])
      }
      }
   else{
    if (tickElement.classList.contains('hidden')) {
      ...
      if(!tickAllElement.classList.contains("hidden")){
       ...
       setSP(selectProduct=>selectProduct.filter(item=>item.id===id))
