...
import { setProduct, setError, setQuantity } from '../../action/client/productDetail';
const ProductDetail= ()=>{
    const dispatch = useDispatch() ;
    const {product,error,quantity}= useSelector(state=>state.ProductDetail);
            ...
            <div className=" h-auto  md:ml-24 mt-8 md:space-y-12 space-y-4 md:text-center">
                ...
                <div className=" fixed p-2 bottom-0 w-full  bg-orange-200 left-0 flex justify-center md:relative">
                    <input
                        ...
                        onChange={changeQuantity} // Hàm xử lý sự kiện
                            />
                    <button className="w-44 h-10  ml-8 text-lg font-medium text-white bg-orange-500 rounded-md" onClick={addToCart}> Mua Ngay</button></div></div>
        </div>
